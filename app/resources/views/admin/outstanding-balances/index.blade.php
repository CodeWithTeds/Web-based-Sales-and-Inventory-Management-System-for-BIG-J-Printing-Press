<x-layouts.app :title="__('Outstanding Balances')">
    <div class="p-4">
                ...
                <tbody>
                    @forelse($orders as $order)
                        <tr class="border-t">
                            ...
                            <td class="px-3 py-2 font-semibold text-red-600">₱{{ number_format($order->computed_remaining, 2) }}</td>
                            <td class="px-3 py-2">{{ optional($order->computed_latest_due)->format('Y-m-d') }}</td>
                            <td class="px-3 py-2">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.outstanding-balances.payments.create', $order) }}" class="text-blue-600">Add Payment</a>
                                    <form method="POST" action="{{ route('admin.outstanding-balances.reminder', $order) }}">
                                        @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-3 py-4 text-center text-zinc-500">No outstanding balances found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
