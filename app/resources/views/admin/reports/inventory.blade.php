...
<x-layouts.app title="{{ $title }}">
    ...
    <!-- Filters -->
    <div class="mb-6 rounded-2xl bg-white p-5 shadow-lg border border-gray-100 dark:border-neutral-700 dark:bg-zinc-900">
        <form method="get" class="grid grid-cols-1 gap-4 md:grid-cols-4">
            <div>
                <label for="from" class="block text-xs font-medium text-gray-600 mb-1 dark:text-neutral-400">{{ __('From') }}</label>
                <input id="from" type="date" name="from" value="{{ request('from') }}" class="w-full rounded-md border border-neutral-300 px-2 py-2 text-sm dark:border-neutral-700 dark:bg-zinc-800 dark:text-neutral-200" />
            </div>
            <div>
                <label for="to" class="block text-xs font-medium text-gray-600 mb-1 dark:text-neutral-400">{{ __('To') }}</label>
                <input id="to" type="date" name="to" value="{{ request('to') }}" class="w-full rounded-md border border-neutral-300 px-2 py-2 text-sm dark:border-neutral-700 dark:bg-zinc-800 dark:text-neutral-200" />
            </div>
            <div class="md:col-span-2 flex items-end gap-2">
                <button type="submit" class="inline-flex items-center gap-2 rounded-md bg-[var(--color-primary)] px-3 py-2 text-sm font-medium text-white hover:brightness-95">{{ __('Apply Filters') }}</button>
                <a href="{{ route('admin.reports.inventory') }}" class="inline-flex items-center gap-2 rounded-md bg-gray-200 px-3 py-2 text-sm font-medium text-gray-800 hover:bg-gray-300 dark:bg-neutral-800 dark:text-neutral-200 dark:hover:bg-neutral-700">{{ __('Reset') }}</a>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-4 mb-6">
        <div class="rounded-2xl bg-white p-4 shadow-lg border border-gray-100 dark:border-neutral-700 dark:bg-zinc-900">
            ...
            <p class="text-2xl font-semibold text-[var(--color-primary)]">{{ ($stockIn ?? collect())->count() }}</p>
        </div>
        <div class="rounded-2xl bg-white p-4 shadow-lg border border-gray-100 dark:border-neutral-700 dark:bg-zinc-900">
            <p class="text-xs text-gray-600 dark:text-neutral-400">{{ __('Product Out Lines') }}</p>
            <p class="text-2xl font-semibold text-[var(--color-primary)]">{{ ($productOut ?? collect())->count() }}</p>
        </div>
        <div class="rounded-2xl bg-white p-4 shadow-lg border border-gray-100 dark:border-neutral-700 dark:bg-zinc-900">
            <p class="text-xs text-gray-600 dark:text-neutral-400">{{ __('Materials In Transactions') }}</p>
            <p class="text-2xl font-semibold text-[var(--color-primary)]">{{ ($stockOut ?? collect())->count() }}</p>
        </div>
        <div class="rounded-2xl bg-white p-4 shadow-lg border border-gray-100 dark:border-neutral-700 dark:bg-zinc-900">
            <p class="text-xs text-gray-600 dark:text-neutral-400">{{ __('Materials Out / Used (Work)') }}</p>
            <p class="text-2xl font-semibold text-[var(--color-primary)]">{{ ($materialsOut ?? collect())->count() }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div class="rounded-2xl bg-white p-6 shadow-lg border border-gray-100 dark:border-neutral-700 dark:bg-zinc-900">
            ...
            <div class="overflow-x-auto rounded-xl ring-1 ring-gray-200 dark:ring-neutral-700">
                <table class="min-w-full text-xs">
                    <thead class="bg-gray-50 dark:bg-zinc-800">
                        <tr class="text-left text-gray-600 dark:text-neutral-300">
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @forelse($stockIn as $txn)
                            <tr class="text-gray-800 dark:text-neutral-200 odd:bg-gray-50 dark:odd:bg-zinc-800">
