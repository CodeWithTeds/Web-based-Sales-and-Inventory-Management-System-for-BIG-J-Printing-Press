    ...
    <div class="py-6">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-zinc-900 shadow-sm sm:rounded-lg p-6 space-y-6 border border-gray-200 dark:border-zinc-700">
                ...
                @if($order->status === 'pending')
                <div class="bg-white dark:bg-zinc-900 p-6 rounded-lg shadow-md mb-6 border border-red-200 dark:border-red-900">
                     <h3 class="text-lg font-medium text-red-800 dark:text-red-400 mb-4">Cancel Request</h3>
                     @php $ordersRoutePrefix = request()->routeIs('staff.orders.*') ? 'staff.orders' : 'admin.orders'; @endphp
                     <form action="{{ route($ordersRoutePrefix . '.cancel', $order) }}" method="POST" onsubmit="return confirm('Are you sure you want to cancel this request?');">
                        @csrf
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Cancellation Reason</label>
                            <textarea name="cancellation_reason" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm" required placeholder="Please provide a reason for cancellation..."></textarea>
                        </div>
                        <div class="flex justify-end">
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">Cancel Request</button>
                        </div>
                     </form>
                </div>
                @endif
